    ...
    try {
        ...
        if (response.ok) { 
            ...
            map.on('load', () => {
                ...
                setZoomMap.addEventListener('click', function(e) {
                    ...
                    document.getElementById('setNewFormMap').addEventListener('click', () => {
                        ...
                        imagenFondo.src = data.marked.properties.image;
            ...
            if(errorObj.code === 'NOTPOINT') {
                ...
                // Aquí agregamos los eventos de la libreria del geocoder
                let geocoderElement = document.querySelector('#mapBoxGeocoder');
                geocoderElement.appendChild(geocoder.onAdd(map));


                let statusGeocoder = false;

                geocoder.on('result', (e) => {

                    geocoder.clear();

                    const resultGeocoder = e.result.geometry;
                    const lngLatGeocoder = [resultGeocoder.coordinates[0], resultGeocoder.coordinates[1]];
                    ...
                    .setLngLat(lngLatGeocoder)
                    .addTo(map);

                    customMarker.getElement().classList.add('mapboxgl_image');
                    

                    const onDragEnd = () => {
                        const lngLat = customMarker.getLngLat();
                        customMarker.setLngLat(map.getCenter());
                        document.getElementById('mapFormLng').value = lngLat.lng;
                        document.getElementById('mapFormLt').value = lngLat.lat;
                    }
                    ...
                    const zoomValidate = () => {
                        ...
                        // Verificar si el nivel de zoom está dentro del rango especificado
                        if (zoom > 14) {
                            customMarker.getElement().querySelector('.mapboxgl_image_light').style.background = '#03C988';
                            customMarker.getElement().querySelector('.mapboxgl_image_tool_tip').style.borderColor = '#03C988 transparent transparent transparent';
                            document.getElementById('setSearchMap').style.pointerEvents = 'initial';
                            document.getElementById('setSearchMap').style.opacity = '1';
                        } else {    
                            customMarker.getElement().querySelector('.mapboxgl_image_light').style.background = '#EB455F';
                            customMarker.getElement().querySelector('.mapboxgl_image_tool_tip').style.borderColor = '#EB455F transparent transparent transparent';
                    ...
                    const handleClick = (e) => {
                        ...
                        if(!statusGeocoder) {
                            ...
                            .then((data) => {
                                ...
                                formDraggable(data.id, customMarker, imageCompress);
                            });
